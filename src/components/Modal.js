...
const Modal = ({product}) => {
  ...
  const createLink = ()=> {
    const shareID = uuid()
    const shareLink = `${window.location.origin}/product/${product.id}?rid=${shareID}`
  ...
  return (
    <>
      ...
      {showModal ? (
        <>
          <div className="mx-auto flex justify-center items-center overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
